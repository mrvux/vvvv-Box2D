...
namespace VVVV 
{
	namespace Nodes 
	{
		...
		void Box2dCreateBodyPersistNode::SetPluginHost(IPluginHost^ Host) 
		{
			this->FHost = Host;
			...
			this->FHost->CreateValueOutput("Is New", 1, nullptr, TSliceMode::Dynamic, TPinVisibility::True, this->vOutIsNew);
		...
		void Box2dCreateBodyPersistNode::Configurate(IPluginConfig^ Input)
		{
		...
		void Box2dCreateBodyPersistNode::Evaluate(int SpreadMax)
		{
			...
			if (dblcreate >= 0.5 && this->vInWorld->IsConnected && this->vInShapes->IsConnected) 
			{
				if (this->mWorld->GetIsValid()) 
				{
					...
					this->FHost->Log(TLogType::Debug,"Create Count" + SpreadMax.ToString());

					for (int i = 0; i < SpreadMax; i++) 
					{
						this->FHost->Log(TLogType::Debug,"Begin Body: " + i.ToString());
						...
						this->FHost->Log(TLogType::Debug,"Current Proxy Count: " + prx.ToString());
						...
						if (testcount)
						{
							this->FHost->Log(TLogType::Debug,"Test Count Passed");
						...
						{
							this->FHost->Log(TLogType::Debug,"Test Count Failed");
						}

						this->FHost->Log(TLogType::Debug,"End Body: " + i.ToString());
			...
			for ( it=this->m_bodies->begin() ; it != this->m_bodies->end(); it++ )
			{
				...
				this->vOutIsNew->SetValue(i,Convert::ToDouble(bdata->Created));
		...
		void Box2dCreateBodyPersistNode::ConnectPin(IPluginIO^ Pin)
		{
		...
		void Box2dCreateBodyPersistNode::DisconnectPin(IPluginIO^ Pin)
		{
			if (Pin == this->vInWorld)
        	{
        		this->mWorld = nullptr;
